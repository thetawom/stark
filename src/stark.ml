...
(* type action = Ast | Sast
   ...
   let () = let lexbuf = Lexing.from_channel stdin in let ast =
   Parser.program Scanner.token lexbuf in

   let sast = Semant.check ast in print_string (Sast.string_of_sprogram
   sast) *)
...
let () =
  ...
  | _ -> (
      ...
      | LLVM_IR ->
          print_string (Llvm.string_of_llmodule (Irgen.translate sast)) )
